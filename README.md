...
- **Optimization conditions:** `baseline`, `kv_cache_quant`, `spec_decode` via `OPTIMIZATION_MODES`. Llama.cpp runs **baseline only**; vLLM and SGLang run each configured mode. Server-side behavior is set through **`VLLM_*_ARGS`**, **`SGLANG_*_ARGS`**, and KV-cache / speculative-decoding env knobs (see `src/config.py`).
...
- **Generation/decoding:** `TEMPERATURE`, `TOP_P`, `MAX_NEW_TOKENS`, `REQUEST_TIMEOUT_SECONDS`; workload size via `LIMIT` / `SMOKE_LIMIT`; optional **`STANDARD_EVAL_*`** for harness tasks.
...
These are the **open-source inference stacks** we deploy behind HF endpoints (container images and CLI flags are wired in `src/config.py` / `src/endpoint_manager.py`):
...
### Open-source software — Python runtime & infra clients
